...
const createChatLi = (message, className) => {
    ...
    let chatContent = className === "outgoing" ? `<p></p>` : `<span class="icon"></span><p></p>`;
...
const generateResponse = (message) => {
    ...
    return fetch(API_URL, requestOptions)
        ...
        .catch(error => {
            ...
            return { response: "Sorry, something went wrong. Please try again later.", isClosingResponse: false };
        });
};
        ...
        generateResponse(userMessage).then(({ response, isClosingResponse }) => {
            ...
            if (isClosingResponse) {
                setTimeout(() => {
                    ...
                    const chatOptionsContainer = createChatLi("", "incoming");
                ...
                confirmationAccepted = false; // Reset confirmation after closing response
            }
        });
    }, 600);
};
...
chatInput.addEventListener("keydown", (e) => {
    if (e.key === "Enter" && !e.shiftKey && window.innerWidth > 700) {
        ...
        if (!confirmationAccepted) {
            confirmationAccepted = confirm("Are you sure you want to continue with this option?");
            if (!confirmationAccepted) {
                alert("Please select an option first.");
                return;
            }
